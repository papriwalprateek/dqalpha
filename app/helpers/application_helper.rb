...
# Using dynamic programming to rank the name tags for given query 
      term.each do |t|
        trank = 0
        for i in 0..(t.length-1)
          ...
          for j in 0..(p.htag.length-1)
            if i == 0 or j == 0
              rankarr[i][j] = 0
            elsif t[i-1].downcase == p.htag[j-1].downcase
  ...
  for i in 0..(rank.length)
    if rank[i] == rank.max
      diff = parsed[i].htag.length - a.length
      if diff == 0
        diff = 0.5
      end
      y[i] = rank[i] * (3.0/diff.abs)  # parameter 3.0 taken to get the best out of best results
    end
  end
  
  l = [] # if l.size == 1 directly use l[0] and feed to algorithm vm. If l.size > 1, show up the results 
  ...
  if y.length == 1
    ...
    l << parsed[x].htag
    puts l[0]
    algorithm_wiki_extract(l[0])
  else
    ymax = y.values.max
    if (ymax/y.values.sum) > 0.6
    l << parsed[y.key(ymax)].htag
    algorithm_wiki_extract(l[0])    
    else
...
# y.sort_by {|key,value| value}
  
puts (Time.now()-t).to_s()+"wiki dynamic algo"
end  
def algorithm_wiki_extract(a)
  if a
